...
export default function LotusParallaxScene() {
  ...
  return (
    <section id="invitation" className="new-hero" onPointerMove={move} onPointerLeave={() => { pointerX.set(0); pointerY.set(0); }}>
      <motion.img src={wedding.assets.environment} alt="Moonlit celestial celebration" className="new-hero__landscape" style={{ x: landscapeX, y: landscapeY }} />
      ...
      <motion.div className="new-hero__copy" initial={reduceMotion ? false : { opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.15, ease: [0.16, 1, 0.3, 1] }}>
        ...
        <div className="new-hero__details">
          ...
          <p>{wedding.venue.name} · Mississauga</p>
        </div>
      </motion.div>

      <motion.img src={wedding.assets.barge} alt={`${wedding.bride} and ${wedding.groom} on a lotus barge`} className="new-hero__barge" style={{ x: subjectX, y: subjectY }} initial={reduceMotion ? false : { opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.5, delay: 0.2, ease: [0.16, 1, 0.3, 1] }} />
      <div className="new-hero__index" aria-hidden>24 · 10 · 26</div>
